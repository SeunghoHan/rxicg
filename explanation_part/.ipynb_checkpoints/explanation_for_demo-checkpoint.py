...
class Explainer():
	...
	def generate_final_output(self, original_cap, caption, relationships, cls_boxes, cls_names, img_path, img_name):
		b_boxes = []
		phrase = []
        
        results = {'type': 'explainer'}
        ...
        result_fr_img = img_name + '_result_fr.jpg'
        fr_save_dir = os.path.join(self.result_dir, img_name)
        if not os.path.exists(fr_save_dir):
            os.makedirs(fr_save_dir)
        results['final_result_dir'] = fr_save_dir
		...
		for i, cap in enumerate(caption):
			...
			if b_boxes[i] is None:
				...
				for j, box in enumerate(bb_boxes): 
					...
					interval += 20
					title = "Intermediate caption: {}".format(original_cap[i][0])
					title += '\n'
            ...
            fr_save_path = os.path.join(self.result_dir, "cap_{}.jpg".format(i))
            fig.savefig(fr_save_path)
            #ax.imsave(final_img.astype(np.uint8))
